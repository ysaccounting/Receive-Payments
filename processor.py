...
EXCLUDE_QBO = {"Y&S Tickets", "The Ticket Guy LLC"}
...
def _clean_customer(v):
    """Remove any '(CAD)', collapse spaces, then append ' (C)'."""
    if pd.isna(v):
        return v
    s = re.sub(r"\(CAD\)", "", str(v), flags=re.I)
...
def build_received_payments(raw):
    """Table1 query -> consolidated Amount Received per affiliate/customer/date."""
    df = raw.copy()
    ...
    # Drop rows with no amount, strip "(CAD)" from customers.
    grouped = grouped[grouped["Amount Received"].notna()]
    grouped["Customer"] = grouped["Customer"].apply(
        lambda v: re.sub(r"\s+", " ", re.sub(r"\(CAD\)", "", str(v), flags=re.I)).strip()
...
def _ledger_by_type(raw, type_value):
    df = raw.copy()
    df["Company"] = df["Company"].astype("string")
    df["Date"] = fix_date(df["Date"])
    df["Name"] = df["Name"].astype("string")
    ...
    df["Company"] = map_company(df["Company"])
...
def build_transfers(raw):
    return _ledger_by_type(raw, "Transfer")


def build_fx(raw):
    return _ledger_by_type(raw, "FX")


def build_cancellation_fees(raw):
    """Table1 (4) -> Cancellation Fees grouped by Company/Date/Type."""
    df = raw.copy()
    df["Company"] = df["Company"].astype("string")
    df["Date"] = fix_date(df["Date"])
    df["Type"] = df["Type"].astype("string")
    ...
    df = df[df["Type"] == "Cancellation Fees"]
    df = df[df["Amount"].notna()]
    df["Company"] = map_company(df["Company"])
    df["Date"] = df["Date"].dt.normalize()
    grouped = (
...
def load_file(file_bytes, filename=""):
    ...
    if fname.endswith(".csv"):
        ...
        # Pick the sheet that actually contains the ledger columns.
...
def process_files(file_list):
    ...
    company_dfs = {name: company_slice(receipts, name) for name in OUTPUT_COMPANIES}

    # Date range from output dates only (transfers excluded), guarding bad years.
    dates = set()
    ...
    # ── One workbook per company (non-empty only) ──────────────────────────
    company_files = {}
    for name, cdf in company_dfs.items():
        if len(cdf) == 0:
            continue
    ...
    # ── Stats for the UI ───────────────────────────────────────────────────
    stats = {"Combined": {"rows": len(receipts),
                          "total": round(float(receipts["Amount Received"].sum()), 2)}}
    for name, cdf in company_dfs.items():
        stats[name] = {
            "rows": len(cdf),
            "total": round(float(cdf["Amount Received"].sum()), 2) if len(cdf) else 0.0,
        }
