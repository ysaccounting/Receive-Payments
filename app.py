  ...
  <div class="card">
    ...
    <section id="panel" class="panel">
      ...
      <div id="results" style="display:none">
        ...
        <table class="summary">
          <thead><tr><th>Company</th><th class="num">Receive Payments</th><th class="num">Negative Payments</th><th class="num">Cancellation Fees</th><th class="num">FX</th></tr></thead>
          <tbody id="rows"></tbody>
        </table>
...
function done(id,j){
  ...
  let rowsHtml=`<tr class="combined-row"><td>Combined</td>${cell(comb.receive)}${cell(comb.negative)}${cell(comb.cancellation)}${cell(comb.fx)}</tr>`;
  j.all_companies.forEach(c=>{
    ...
    rowsHtml+=`<tr class="${blank?'zero':''}"><td>${c}</td>${cell(s.receive)}${cell(s.negative)}${cell(s.cancellation)}${cell(s.fx)}</tr>`;
  });
