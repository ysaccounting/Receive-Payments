...
function done(id,j){
  ...
  if(j.mode==='tv'){
    ...
    document.getElementById('zipSub').textContent='Includes the Receive Payments and All Tabs files';
    ...
    grid+=`<a class="fcard has" href="/download/${id}/combined"><span class="fcard-ico">${FILEICO}</span><span class="fcard-name">All Tabs</span></a>`;
...
def _combined_name(meta):
    dr = meta["date_range"]
    if meta.get("mode") == "tv":
        return f"All Tabs - {meta.get('broker','TV')} - {dr}.xlsx"
